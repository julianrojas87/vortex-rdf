...
| Var | Default | Meaning |
| --- | --- | --- |
| `BENCH_DIM` | 128 | dataset is `D³` rows |
...
The two ratios decide how much of the dataset is *terms*: the dictionary holds `subjectRatio + objectRatio` of them per quad. At the defaults that is 1,048,576 triples over ~630k distinct terms, ten triples describing each subject, and probes spanning five orders of magnitude of selectivity (1 row for a fully-bound pattern, ~10 for a subject, ~3% for a predicate).
